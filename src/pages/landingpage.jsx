...
function ProductSkeleton() {
	return (
		<div className="min-w-65 max-w-65 rounded-3xl bg-white border border-secondary/10 shadow-sm overflow-hidden animate-pulse">
			<div className="h-55 bg-primary/70" />
			<div className="p-4">
...
const getSalePrice = (product) => Number(product?.price || 0);
const getListPrice = (product) =>
	Number(product?.labeledPrice ?? product?.labelledPrice ?? 0);

function PriceBlock({ price, labelledPrice }) {
	const salePrice = Number(price || 0);
	const listPrice = Number(labelledPrice || 0);
	const hasDiscount =
		listPrice > 0 &&
		salePrice > 0 &&
		listPrice > salePrice;
...
function ProductSlideCard({ product }) {
	...
	return (
		<div className="min-w-65 max-w-65 snap-start rounded-3xl bg-white border border-secondary/10 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden group">
			<div className="relative h-55 bg-linear-to-br from-primary to-white flex items-center justify-center overflow-hidden">
				{discount > 0 && (
				...
				<img
					src={image}
					alt={product?.name}
					className="h-45 w-45 object-contain group-hover:scale-105 transition-transform duration-300"
				/>
			</div>
			...
			<div className="p-4">
				...
				<div className="mt-4 flex gap-2">
					<Link
						to={`/products`}
						className="flex-1 h-11 rounded-xl bg-secondary text-white flex items-center justify-center font-semibold hover:bg-secondary/90 transition"
					>
						View
					</Link>
					<Link
						to={`/products`}
						className="flex-1 h-11 rounded-xl border border-accent text-accent flex items-center justify-center font-semibold hover:bg-accent hover:text-white transition"
					>
						Buy Now
					</Link>
				</div>
	...
	const dealProducts = useMemo(() => {
		return [...products]
			.filter(
				(item) =>
					getListPrice(item) > 0 &&
					getListPrice(item) > getSalePrice(item),
			)
			.sort(
				(a, b) =>
					(getListPrice(b) - getSalePrice(b)) -
					(getListPrice(a) - getSalePrice(a)),
			)
			.slice(0, 10);
	}, [products]);
	...
	return (
		<div className="w-full min-h-screen bg-primary">
			...
			<section className="relative overflow-hidden">
				<div className="absolute inset-0 bg-linear-to-br from-secondary via-secondary to-accent" />
				<div className="absolute -top-20 -right-20 w-70 h-70 rounded-full bg-white/10 blur-3xl" />
				<div className="absolute bottom-0 left-0 w-65 h-65 rounded-full bg-accent/20 blur-3xl" />
					...
					<div className="grid lg:grid-cols-2 gap-10 items-center">
						<div>
							...
							<div className="mt-8 flex flex-wrap gap-4">
								<Link
									to="/products"
									className="h-13 px-7 rounded-2xl bg-accent text-white font-bold flex items-center justify-center hover:bg-white hover:text-secondary transition"
								>
									Shop Now
								</Link>
								<Link
									to="/about"
									className="h-13 px-7 rounded-2xl border border-white/25 text-white font-bold flex items-center justify-center hover:bg-white hover:text-secondary transition"
								>
									Learn More
								</Link>
							</div>
						...
						<div className="relative">
							<div className="relative rounded-4xl border border-white/15 bg-white/10 backdrop-blur-xl p-5 shadow-2xl">
								<div className="grid grid-cols-2 gap-4">
									...
									<div className="col-span-2 rounded-3xl bg-white p-5 shadow-sm">
										<div className="flex flex-col sm:flex-row items-center gap-5">
											<div className="w-full sm:w-55 h-45 rounded-3xl bg-primary flex items-center justify-center overflow-hidden">
												<img
													src={
											...
											<div className="flex-1 w-full">
												...
												<div className="mt-4">
													<Link
														to="/products"
														className="inline-flex h-11.5 px-6 rounded-xl bg-secondary text-white font-bold items-center justify-center hover:bg-accent transition"
													>
														Explore Products
													</Link>
												</div>
			...
			<section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
				<div className="rounded-[36px] bg-linear-to-r from-secondary to-accent p-8 lg:p-12 shadow-xl">
					<div className="grid lg:grid-cols-2 gap-8 items-center">
						...
						<div className="flex lg:justify-end">
							<Link
								to="/products"
								className="h-13.5 px-8 rounded-2xl bg-white text-secondary font-black flex items-center justify-center hover:bg-primary transition"
							>
								Browse All Products
							</Link>
						</div>
